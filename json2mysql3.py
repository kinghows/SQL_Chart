...
if __name__=="__main__":
    ...
    for row in nodes:
        id = row['id']
        name = row['label']
        symbolsize = str(row['size'])
        x = str(row['x'])
        y = str(row['y'])
        attributes = str( {"normal": {"color": row['color']}})
        query ='insert into chart_demo.t_npmdepgraph_nodes(id,name,symbolsize,attributes,x,y) values ("'+id+'","'+name+'","'+symbolsize+'","'+attributes+'","'+x+'","'+y+'")'
    ...
    for row in links:
        source = row['sourceID']
        target = row['targetID']
        query ='insert into chart_demo.t_npmdepgraph_links(source,target) values ("'+source+'","'+target+'")'
        cursor.execute(query.encode('utf-8'))
    query ='commit;'
    cursor.execute(query.encode('utf-8'))
    cursor.close()
