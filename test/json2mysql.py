...
if __name__=="__main__":
    ...
    for row in nodes:
        ...
        value = str(row['value'])
        query ='insert into chart_demo.t_weibo_nodes(name,symbolsize,category,draggable,label,value) values ("'+name+'","'+symbolsize+'","'+category+'","'+draggable+'","'+label+'","'+value+'")'
    ...
    for row in links:
        source = row['source']
        target = row['target']
        query ='insert into chart_demo.t_weibo_links(source,target) values ("'+source+'","'+target+'")'
        cursor.execute(query.encode('utf-8'))
    query ='commit;'
    cursor.execute(query.encode('utf-8'))
    for row in categories:
        name = row['name']
        query ='insert into chart_demo.t_weibo_categories(name) values ("'+name+'")'
        cursor.execute(query.encode('utf-8'))
    query ='commit;'
    cursor.execute(query.encode('utf-8'))
    cursor.close()
